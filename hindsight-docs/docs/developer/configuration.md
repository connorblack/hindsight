...
| `HINDSIGHT_API_CONSOLIDATION_SCOPE_CONCURRENCY` | Concurrent facts allowed per write-scope during parallel consolidation. Only active when `CONSOLIDATION_LLM_PARALLELISM` > 1. `1` (default) = serial merge-as-you-go: each fact sees prior facts' observations and UPDATE-merges into them. `K`>1 lets up to K groups per scope consolidate concurrently for bulk parallel runs — a deliberate speed-vs-strict-merge tradeoff: concurrent same-scope groups can race (last-writer-wins on a shared observation's text and source ids, transient per-scope cap overshoot), reconciled best-effort by the dedup pass and future rounds. Use `1` where strict merge fidelity matters. | `1` |
| `HINDSIGHT_API_CONSOLIDATION_LLM_EXTRA_BODY` | Per-op override of `LLM_EXTRA_BODY` for the consolidation LLM only (e.g. reasoning sampling + a vLLM thinking-budget via `vllm_xargs`). JSON dict merged into the consolidation chat-completion call; falls back to `LLM_EXTRA_BODY` when unset. Env-only (not per-bank configurable, to keep consolidation tunables single-source in env). | `unset` |
