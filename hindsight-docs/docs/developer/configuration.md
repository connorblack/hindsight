...
| `HINDSIGHT_API_CONSOLIDATION_MAX_INPUT_TOKENS` | Max input-token budget for the consolidation LLM prompt. The prompt compactor keeps the assembled prompt (system + observations + source facts, including the response-contract reserve) under this cap; a context-length 400 marks the fact failed once instead of retrying. | `32000` |
